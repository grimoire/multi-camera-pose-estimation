                    ...
                    const int deformable_group)
{
    ...
    int old_device = -1;
    THCudaCheck(cudaGetDevice(&old_device));
    if(old_device==input.device().index()){
        old_device = -1;
    }else{
        THCudaCheck(cudaSetDevice(input.device().index()));
    }
    ...
    auto columns = at::zeros({batch, channels * kernel_h * kernel_w, 1 * height_out * width_out}, input.options());
    ...
    // at::native::baddbmm__cuda(output, ones, bias, 1, 0);
    ...
    // at::native::baddbmm__cuda(output, columns, weight, 1, 1);
    ...
    if(old_device>=0){
        THCudaCheck(cudaSetDevice(old_device));
    }
    return output;
}
